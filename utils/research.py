...
def build_research_summary(
    ...
    summary = {
        ...
        'blind_spot_top_missingness': {str(k): round(float(v), 3) for k, v in zip(blind_spots['field'].head(10), blind_spots['missing_rate'])},
        ...
        'provenance_mean_availability': {str(k): round(float(v), 3) for k, v in zip(provenance_summary['provenance'], provenance_summary['mean_availability_rate'])},
        ...
        'source_disagreement_summary': safe_int_dict(dict(zip(source_disagreement_summary['field'], source_disagreement_summary['disagreement_count']))) if not source_disagreement_summary.empty else {},
        'movement_inconsistency_diagnosis': safe_int_dict(dict(zip(movement_inconsistency_summary['movement_inconsistency_diagnosis'], movement_inconsistency_summary['count']))) if not movement_inconsistency_summary.empty else {},
        'blame_evidence_strength_distribution': safe_int_dict(dict(zip(blame_evidence_strength_distribution['blame_evidence_strength'], blame_evidence_strength_distribution['count']))) if not blame_evidence_strength_distribution.empty else {},
