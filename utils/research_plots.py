...
def _update_axis_labels(fig: go.Figure, *, x: str | None = None, y: str | None = None, legend: str | None = None) -> go.Figure:
    if x is not None:
...
def create_blind_spot_figure(df: pd.DataFrame, fields: list[str]) -> go.Figure:
    ...
    for field in fields:
        if field in df.columns:
            missing = int(df[field].astype(str).str.lower().isin({'na', 'none', 'nan', 'null', ''}).sum())
        else:
            missing = total
        rows.append({'field': humanize_field_name(field), 'missing_rate': missing / total})
...
def create_accountability_by_taxonomy_figure(df: pd.DataFrame, top_n: int = 8) -> go.Figure:
    ...
    counts = working.groupby(['scenario_class', 'blame_group']).size().reset_index(name='count')
...
def create_taxonomy_by_road_user_figure(df: pd.DataFrame, top_n: int = 8) -> go.Figure:
    top_taxonomy = df['scenario_class'].astype(str).value_counts().head(top_n).index.tolist()
    pivot = (
...
def create_provenance_availability_figure(df: pd.DataFrame) -> go.Figure:
    rows = []
    for field in ['form_field_rate', 'checkbox_field_rate', 'narrative_field_rate', 'online_field_rate']:
        if field in df.columns:
            rows.append({
                'provenance': humanize_text(field.replace('_field_rate', '')),
                'availability_rate': float(pd.to_numeric(df[field], errors='coerce').mean()),
            })
...
def create_context_gap_figure(df: pd.DataFrame) -> go.Figure:
    frame = pd.DataFrame([
        {'context_type': humanize_field_name('coarse_context_score'), 'mean_score': float(pd.to_numeric(df['coarse_context_score'], errors='coerce').mean())},
        {'context_type': humanize_field_name('fine_context_score'), 'mean_score': float(pd.to_numeric(df['fine_context_score'], errors='coerce').mean())},
        {'context_type': humanize_field_name('report_explicitness_score'), 'mean_score': float(pd.to_numeric(df['report_explicitness_score'], errors='coerce').mean())},
    ])
...
def create_consistency_figure(df: pd.DataFrame) -> go.Figure:
    ...
    counts['movement_consistency_overall'] = _format_series(counts['movement_consistency_overall'])
...
def create_determinability_figure(df: pd.DataFrame) -> go.Figure:
    ...
    counts['scenario_determinability_group'] = _format_series(counts['scenario_determinability_group'])
...
def create_environment_profile_figure(df: pd.DataFrame) -> go.Figure:
    ...
    counts['environment_friction_profile'] = _format_series(counts['environment_friction_profile'])
...
def create_blame_alignment_figure(df: pd.DataFrame) -> go.Figure:
    ...
    counts['blame_confidence_alignment'] = _format_series(counts['blame_confidence_alignment'])
...
def create_intersection_detail_figure(df: pd.DataFrame) -> go.Figure:
    ...
    counts['intersection_detail_quality'] = _format_series(counts['intersection_detail_quality'])
